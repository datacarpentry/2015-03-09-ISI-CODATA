...
```
   ...
   Can you fix the R code above statement so that the fields of `country_climate` have the
...
2. The function `nrow()` on a `data.frame` returns the number of rows. Use it,
...
fruit_info[order(fruit_info$quantity),]  # sort fruit_info by quantity column
```
...
Previous: [Starting with data](02-starting-with-data.html)  Next: [Manipulating data](04-manipulating-data.html)
